...
Интеграция:
    - Router Agent использует db_hints для выбора БД
    - SQL Generator использует intent для формирования промпта
"""
import logging
import re
...
class QueryUnderstandingAgent:
    ...
    # 🔥 Database routing keywords
    DB_KEYWORDS = {
        "movie": ["movie", "film", "фильм", "кино", "director", "режиссёр", "actor", "актёр", "actress"],
        "music": ["music", "song", "album", "artist", "музык", "песн", "альбом", "исполнитель", "трек"],
        "user": ["user", "customer", "client", "пользователь", "клиент", "покупатель"],
        "rating": ["rating", "review", "рейтинг", "отзыв", "оценка", "звезд", "stars"],
        "genre": ["genre", "жанр", "category", "категория", "type", "тип"],
    }
    ...
    def __init__(self, schema_hints: Optional[Dict[str, Any]] = None) -> None:
        ...
        self._table_names: Set[str] = set(self.schema_hints.get("tables", []))
        self._column_names: Set[str] = set(self.schema_hints.get("columns", []))
    ...
    def analyze(self, query: str) -> QueryUnderstanding:
        ...
        Pipeline:
            ...
            3. Detect tables
            4. Detect filters
            5. Detect aggregations
            6. Extract ORDER BY / LIMIT
            7. Generate DB routing hints
        ...
        # Step 6: ORDER BY / LIMIT / GROUP BY
        ...
        logger.info(f"Query analyzed: intent={result.intent.value}, entities={len(result.entities)}, tables={len(result.tables)}")
        return result
    ...
    def _classify_intent(self, query: str) -> QueryIntent:
        ...
        if "group" in query_lower or "by" in query_lower or "each" in query_lower or "по " in query_lower:
            return QueryIntent.GROUP

        # Check for JOIN indicators
        if "join" in query_lower or "between" in query_lower or "across" in query_lower or "и " in query_lower:
            return QueryIntent.JOIN

        # Check for filter indicators
        if "where" in query_lower or "with" in query_lower or "that" in query_lower or "с " in query_lower:
    ...
    def _extract_order_by(self, query: str) -> Optional[Dict[str, Any]]:
        ...
        if "rating" in query_lower or "рейтинг" in query_lower:
            order_column = "rating"
    ...
    def _generate_db_hints(self, query: str, understanding: QueryUnderstanding) -> List[str]:
        ...
        # 🔥 Улучшенная логика: используем DB_KEYWORDS
        for db_keyword, keywords in self.DB_KEYWORDS.items():
            if any(kw in query_lower for kw in keywords):
                hints.append(db_keyword)

        # Добавляем hints из detected tables
        for table in understanding.tables:
            table_lower = table.lower()
            for db_keyword, keywords in self.DB_KEYWORDS.items():
                if any(kw in table_lower for kw in keywords):
                    if db_keyword not in hints:
                        hints.append(db_keyword)
        ...
        logger.debug(f"DB hints generated: {hints}")
        return hints
    ...
    def _calculate_confidence(self, understanding: QueryUnderstanding) -> float:
        ...
        # Lower confidence if no filters for filter query
        if understanding.intent == QueryIntent.FILTER and not understanding.filters:
            confidence -= 0.1
    ...
    def to_llm_context(self, understanding: QueryUnderstanding) -> str:
        """
        Конвертировать анализ в контекст для LLM.

        Args:
            understanding: Результат анализа.
        ...
        parts.append(f"Intent: {understanding.intent.value}")

        if understanding.tables:
            parts.append(f"Tables: {', '.join(understanding.tables)}")

        if understanding.aggregations:
            parts.append(f"Aggregations: {', '.join(understanding.aggregations)}")

        if understanding.filters:
            filter_strs = [f"{f['type']}:{f['value']}" for f in understanding.filters]
            parts.append(f"Filters: {', '.join(filter_strs)}")

        if understanding.order_by:
            parts.append(f"Order: {understanding.order_by['column']} {understanding.order_by['direction']}")

        if understanding.limit:
            parts.append(f"Limit: {understanding.limit}")

        if understanding.group_by:
            parts.append(f"Group By: {', '.join(understanding.group_by)}")
